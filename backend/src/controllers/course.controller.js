...
// ⭐ Helper function to populate course details
const populateCourse = (query) => {
    return query.populate("subjects", "subjectName subjectCode"); // Populate subject name and code
};


// ⭐ Controller to add a new course
const addCourse = asyncHandler(async (req, res) => { // Corrected argument order
  ...
  if (
    [courseCode, courseName, academicYear, semester, courseCredit].some(
     ...
     const missingField = [courseCode, courseName, academicYear, semester, courseCredit].find(
         (field) =>
        field === undefined ||
        field === null ||
        (typeof field === "string" && field.trim() === "")
  ...
  if (existCourse) {
    ...
    let errorMessage = "Course already exists.";
     if (existCourse.courseCode === courseCode.trim().toUpperCase()) errorMessage = `Course with code '${courseCode.trim().toUpperCase()}' already exists.`;
     else if (existCourse.courseName === courseName.trim()) errorMessage = `Course with name '${courseName.trim()}' already exists.`;
  ...
  const course = await Course.create({
    ...
    academicYear: academicYear.trim(), // Trim year
    semester: semester.trim(), // Trim semester (ensure casing matches schema enum if applicable)
  ...
  const createdCourse = await populateCourse(
...
const getAllCourses = asyncHandler(async (req, res) => {
    ...
    const allCourses = await populateCourse(Course.find({})); // Fetch all and populate
    ...
    return res
        .status(200)
        .json(
            new Apiresponse(200, allCourses, "Courses fetched successfully.")
        );
});
...
const getCourseById = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new Apiresponse(200, course, "Course found successfully.")); // Corrected message
});
...
const updateCourse = asyncHandler(async (req, res) => {
  ...
  if (!(courseCode || courseName || academicYear || semester || subjects || courseCredit)) {
       throw new ApiError(400, "Please provide at least one field to update.");
  }

  // Validate courseCredit if provided
   if (courseCredit !== undefined && (typeof courseCredit !== 'number' || courseCredit < 0)) {
       throw new ApiError(400, "Course credit must be a non-negative number if provided.");
  }

   // Validate subjects if provided
  if (subjects !== undefined && !Array.isArray(subjects)) {
      throw new ApiError(400, "Subjects must be provided as an array if updating.");
  }
   // Further validation: check if subject IDs are valid ObjectIds if needed
  ...
  const updatedCourse = await Course.findByIdAndUpdate(
    courseId, // Find by ID from params
    {
      $set: { // Use $set to update specific fields
        ...
        ...(academicYear && { academicYear: academicYear.trim() }), // Update year
        ...(semester && { semester: semester.trim() }), // Update semester
  ...
  ).populate("subjects", "subjectName subjectCode"); // Populate subject details in the returned document
